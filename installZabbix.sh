...
apt install -y mysql-server zabbix-server-mysql zabbix-frontend-php zabbix-apache-conf zabbix-sql-scripts zabbix-agent
...
flush privileges;
MYSQL_SCRIPT
#Descomprimios uno de los ficheros que viene con los paquetes de zabbix
...
EOF
#Instalamos la lengua inglesa en las locales del equipo para soluciona un error que puede 
